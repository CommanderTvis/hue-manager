...
/**
 * Simple time-based rate limiter with minimum delay between calls.
 * Used for operations that should be called infrequently:
 * - Discovery: 5 second minimum delay
 * - Linking/validation: 1 second minimum delay
 *
 * This implementation uses monotonic time to avoid issues with system clock changes.
 */
class MinimumDelayRateLimiter(
    private val minimumDelay: Duration
) {
    ...
    private suspend fun awaitMinimumDelay() {
        val waitTime = mutex.withLock {
            val mark = lastCallMark
            val waitNeeded = if (mark == null) {
                Duration.ZERO
            } else {
            ...
            // Update last call time now to prevent other coroutines from
            // calculating the same wait time
